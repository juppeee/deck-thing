...
EN = {
    ...
    # Spotify
    "Titel, Cover und Steuerung laufen ohne Anmeldung. Für Like, deine Bibliothek, Smart Shuffle und Spotifys eigenen Lautstärkeregler verbindest du dein Konto einmal.":
        "Titles, covers and playback controls work without signing in. For likes, your library, Smart Shuffle and Spotify's own volume control, connect your account once.",
    ...
    "Trennen": "Disconnect",
    "<b>Spotify-Entwicklerseite öffnen</b> und mit deinem Konto anmelden. Voraussetzung ist Spotify Premium.":
        "<b>Open the Spotify developer site</b> and sign in with your account. Spotify Premium is required.",
    "developer.spotify.com öffnen": "Open developer.spotify.com",
    "<b>„Create app“</b> klicken. Name und Beschreibung sind egal, z. B. „Deck Thing“.":
        "Click <b>“Create app”</b>. Name and description don't matter, e.g. “Deck Thing”.",
    "Diese <b>Redirect URI</b> eintragen und auf „Add“ klicken:": "Enter this <b>Redirect URI</b> and click “Add”:",
    "Kopieren": "Copy",
    "Bei „Which API/SDKs“ <b>Web API</b> ankreuzen, Bedingungen annehmen, speichern.":
        "Under “Which API/SDKs” tick <b>Web API</b>, accept the terms, save.",
    ...
    "„Automatisch“ richtet sich nach Windows: deutsche Oberfläche auf deutschen Systemen, sonst Englisch.":
        "“Automatic” follows Windows: German on German systems, English otherwise.",
    ...
    "Das Gerät bleibt verbunden, auch wenn das Fenster zu ist. Beenden über das Symbol unten rechts.":
        "The device stays connected even with the window closed. Quit from the icon in the bottom right.",
    ...
    "Entfernen": "Remove",
    "So sieht die Tastenseite auf dem Gerät aus – sichtbar sind 4 × 2, der Rest wird gewischt.":
        "This is how the key page looks on the device – 4 × 2 are visible, swipe for the rest.",
    "Taste 1": "Key 1",
    "Taste {n}": "Key {n}",
    ...
    "Medien": "Media",
    "Diese Tasten kennt Windows, auf keiner Tastatur gibt es sie – sie kommen also nichts anderem in die Quere. Im Programm (OBS, Discord, …) ins Feld fürs Tastenkürzel klicken und dann die Taste am Gerät drücken. Grau = schon vergeben.":
        "Windows knows these keys, but no keyboard has them – so they never get in the way. In the program (OBS, Discord, …) click the shortcut field, then press the key on the device. Grey = already taken.",
    ...
    "„Aufnehmen“ drücken, dann die Tasten. Möglich sind ctrl, shift, alt, win, a–z, 0–9, f1–f24, esc, enter, tab, space, Pfeile, volumemute …":
        "Press “Record”, then the keys. Possible: ctrl, shift, alt, win, a–z, 0–9, f1–f24, esc, enter, tab, space, arrows, volumemute …",
    ...
    "Mischpult: senkrechte Regler nebeneinander, seitlich wischen. Liste: eine Zeile pro App, nach unten scrollen.":
        "Mixer: vertical faders side by side, swipe sideways. List: one row per app, scroll down.",
    "Mischpult": "Mixer",
    "Liste": "List",
...
JS = Path(__file__).resolve().parent.parent / "web" / "i18n.js"


def main() -> None:
    src = JS.read_text(encoding="utf-8")
    head, rest = src.split("/*EN-START*/", 1)
