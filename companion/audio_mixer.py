"""Volume mixer for the device's audio page: master, microphone, every app and the output device.

Reads the same as the Windows volume mixer (Core Audio via pycaw). pycaw speaks COM, so everything
runs on its own thread with CoInitialize. Program icons come straight from the .exe via Windows.
Switching the output device goes through IPolicyConfig – not officially documented, but unchanged
since Windows 7 and used the same way by EarTrumpet and SoundSwitch.
"""
...
ICON_SIZE = 44
RECENT_SOUND_SECONDS = 20  # an app counts as "playing" and stays in front for this long
SESSIONS_SECONDS = 2.0     # look for new apps this often; levels and volumes every time
DEVICES_SECONDS = 10.0     # device list less often: it takes half a second and the levels would stutter
SILENT_PEAK = 0.002
...
def exe_icon(path: str, size: int = ICON_SIZE) -> Image.Image | None:
    """The .exe's program icon at the requested size, with transparency (RGBA)."""
    ...
    if not alpha.getbbox():
        img.putalpha(255)  # old icon without an alpha channel
        return img
    # DrawIconEx returns premultiplied colours – undo that, or edges turn dark
    px = img.load()
    for y in range(size):
        for x in range(size):
            r, g, b, a = px[x, y]
...
def icon_color(img: Image.Image) -> str:
    """A strong base colour of the icon for the tinted tile."""
...
def file_description(path: str) -> str | None:
    """"Description" from the file properties, e.g. "Google Chrome" instead of chrome.exe."""
    try:
        ...
        if not _version.VerQueryValueW(data, key, ctypes.byref(pointer), ctypes.byref(length)) or not length.value:
            return None
        text = ctypes.wstring_at(pointer).strip()  # up to the null; length is wrong for some files
        return text or None
    except OSError:
        return None
...
def device_name(friendly: str | None) -> str:
    """"Lautsprecher (2- USB Audio Device)" → "Lautsprecher (USB Audio Device)" – Windows adds the number
    only to tell identical drivers apart, and it means nothing to anyone."""
...
class AudioMixer:
    def __init__(self) -> None:
        ...
        self.icons: dict[str, bytes] = {}      # id → RGB565A8 with width/height in front
        self._app_info: dict[str, tuple[str, str | None, str]] = {}  # exe → (name, icon_id, colour)
        self._last_sound: dict[str, float] = {}
        self._outputs: dict[str, str] = {}     # short id → Windows device id
        self._output_list: list[dict] = []
        self._apps: dict[str, dict] = {}       # app id → name, icon, sessions and level meters
    ...
    def _init_com() -> None:
        ...
        warnings.filterwarnings("ignore", module="pycaw")  # devices lacking some properties complain loudly
    ...
    def _meter(device):
        ...
        # QueryInterface rather than ctypes.cast: cast doesn't count the reference, and the double Release crashes
    ...
    def _refresh_devices(self) -> None:
        """Output device, microphone and device list (slow: Windows queries every device on its own)."""
    ...
    def _refresh_sessions(self) -> None:
        """Which apps have an audio session right now (fast enough for every two seconds)."""
        from pycaw.pycaw import AudioUtilities, IAudioMeterInformation

        apps: dict[str, dict] = {}
        for session in AudioUtilities.GetAllSessions():
            if session.State == 2:  # expired
                continue
    ...
    def _snapshot(self) -> dict:
        """Volumes and levels from the remembered sessions (fast, for ten frames per second)."""
        now = time.monotonic()
        ...
        def endpoint(ch):
            ...
            try:
                ...
                self._dirty = True  # device gone – collect again next time
                return None
        ...
        for app in self._apps.values():
            try:
                ...
                self._dirty = True  # session ended
                continue
            if peak > SILENT_PEAK:
    ...
    def _set_output(self, output: str) -> None:
        ...
        # like choosing in Windows: for media, games and calls at once
